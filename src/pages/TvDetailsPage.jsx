  ...
  useEffect(() => {
    const fetchTvData = async () => {
      try {
        ...
        const tvRes = await fetchTvShowDetails(tv_id);
        setTvShow(tvRes);

        const creditsRes = await tmdb.get(`/tv/${tv_id}/credits`);
        setCredits(creditsRes.data);

        const similarRes = await tmdb.get(`/tv/${tv_id}/similar`);
        setSimilar(similarRes.data.results);

        const videosRes = await tmdb.get(`/tv/${tv_id}/videos`);
        setVideos(videosRes.data.results);

        const watchProvidersRes = await tmdb.get(`/tv/${tv_id}/watch/providers`);
        setWatchProviders(watchProvidersRes.data.results);

        const keywordsRes = await tmdb.get(`/tv/${tv_id}/keywords`);
        setKeywords(keywordsRes.data.keywords);

        const recommendationsRes = await tmdb.get(`/tv/${tv_id}/recommendations`);
